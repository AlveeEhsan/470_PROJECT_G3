...
class RecipeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        "recipe_description"=>"required|max:20000",
        "ing"=>"required|max:10",
        "image"=>"required",
    ...
    public function update(Request $request, $id)
    {
        ...
        $input=Recipes::find($id);
        $input->recipe_name=$request->recipe_name;
        $input->category=$request->category;
        $input->cookingtime=$request->cookingtime;
        $input->allergy=$request->allergy;
        $input->video=$request->video;
        $input->nutrition=$request->nutrition;
        $input->link=$request->link;
        $input->recipe_description=$request->recipe_description;

        if ($request->ing) {
            $ingredients=$request->ing;
            $short=implode(",",$ingredients);
            $input->ing=$short;}

                        ...
                        <div class="innerimage">
                                ...
                                <div class="card-body">
                                  ...
                                  <div class='card-text'>
                                      ...
                                      <button class="btn btn-success" type="button" data-bs-toggle="offcanvas" data-bs-target="#staticBackdrop{{$key+1}}" aria-controls="staticBackdrop">View Details</button>
                                      
                                      <div class="offcanvas offcanvas-start" data-bs-backdrop="static" tabindex="-1" id="staticBackdrop{{$key+1}}" style="width: 30%" aria-labelledby="staticBackdropLabel">
                                        <div class="offcanvas-header">
                                          <h5 class="offcanvas-title" id="staticBackdropLabel">Recipe Description</h5>
                                          @if(Auth::user()->favoriteRecipes->contains($item->id))
                                          <form action="{{ route('recipes.removeFromFavorites', ['recipe' => $item->id]) }}" method="POST">
                                              @csrf
                                              <small><button class="btn btn-primary btn-sm" type="submit"><i class="fa fa-close"></i>Remove from Favorites</button></small>
                                          </form>
                                          @else 
                                          <form action="{{ route('recipes.addToFavorites', ['recipe' => $item->id]) }}" method="POST">
                                              @csrf
                                              <small><button class="btn btn-primary btn-sm" type="submit"><i class="fa fa-angellist"></i>Add to Favorites</button></small>
                                          </form>
                                          @endif
                                          ...
                                          <div class='off-card'>
                                                  ...
                                                  <div class='offcanvas-box'>
                                                    <p> Cautionary Information: </p>
                                                      <h5 class="offcard-title">{{$item->allergy}}</h5>                                           
                                                  </div>
                                                  <div class='offcanvas-box'>
                                                    <p><a href="{{$item->link}}">External Link</a></p>
                                                  </div>
                                                  <div class='offcanvas-box'>
                                                    <p> Video Tutorial: </p>
                                                  ...
                                                  <div class='offcanvas-box'> 
                                                    <div class="span4">
                                                        <p>Write a Review</p>
                                                        <form method="POST" action="{{url('add_rating')}}" name="ratingform_{{$item->id}}" id="ratingform_{{$item->id}}">@csrf
                                                            <input type="hidden" name="recipe_id" value="{{$item->id}}">
                                                            <div class="rate">
                                                                <input type="radio" id="star5_{{$item->id}}" name="rating" value="5" />
                                                  ...
                                                  @foreach ($rating as $ki=>$rate)
                                                        ...
                                                        <p>{{$rate->user->name}} &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <small style="text-align:right" >  Reviewed on {{date("d M Y H:i:s",strtotime($rate->created_at));}}</small><br>
                                                        <?php
                                                        $count=1;
                                                        while($count<=5){ 
                            ...
                            <div class="card-footer">
                                <small class="text-body-secondary">{{$item->ing}}</small>
